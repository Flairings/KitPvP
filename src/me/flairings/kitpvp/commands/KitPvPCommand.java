    ...
    public boolean onCommand(CommandSender commandSender, Command cmd, String label, String[] args) {
        if (args.length < 1) {
            if (commandSender.hasPermission("kitpvp.command.helpcommand")) {
                ...
                commandSender.sendMessage(CC.translate(Main.getInstance().getConfig().getString("No-Permissions-Message").replace("{PREFIX}", "" + (CC.translate(Main.getInstance().getConfig().getString("Prefix"))))));
            }
            return false;
        } else {
            if (args[0].equalsIgnoreCase("howtoplay")) {
                if (commandSender.hasPermission("kitpvp.helpcommand.howtoplay")) {
                    commandSender.sendMessage(CC.translate("\n" +
                            ...
                            "&6/howtoplay <player>&7┃ &fDisplays servers help menu for another player \n" +
                            "&7&m------------------------------------------- \n".replace("{PREFIX}", "" + (CC.translate(Main.getInstance().getConfig().getString("Prefix"))))));
                } else {
                    commandSender.sendMessage(CC.translate(Main.getInstance().getConfig().getString("No-Permissions-Message").replace("{PREFIX}", "" + (CC.translate(Main.getInstance().getConfig().getString("Prefix"))))));
                }
